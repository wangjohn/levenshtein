...
Use the [runtime prerequisites](setup.md#prerequisites): Go for the source launcher, and a Docker-compatible runtime for checks bound to a Dagger environment. The SDK provisions the pinned Dagger CLI. Keep the application and Levenshtein in separate directories:
