...
These kinds take no options of their own on either executor except `go-lint`'s [`lint` object](#lint-selection), which works the same way here except that [community rules](#community-rule-modules) are skipped with a warning, and `go-imports`'s [`imports` object](#import-rules), which works the same way here; a `command` or `semantic` object is rejected. The environment may still declare `identity`, `env`, `pass_env`, and pinned `tools`. `go-http`, `go-sql`, and Levenshtein's own `self-test` remain Dagger-only, and Dagger stays the default for a repository with no configuration file.
