...
Use a running Docker-compatible container runtime and Dagger **0.21.9**. Install the pinned CLI with the checked-in archive checksums:
...
`--dry-run` still needs Dagger and its engine. A run name selects checks; it does not switch Git branches or fetch code. The passed source directory is what gets verified. CI supplies the PR/merge candidate or default-branch checkout.
