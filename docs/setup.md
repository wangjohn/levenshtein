...
Generated SDK (`runner/dagger.gen.go`, `runner/internal/dagger`, `runner/internal/telemetry`) uses **exact** key `dagger-sdk-v2-…` (no `restore-keys`). Restore + save are separate steps; save runs only when `scripts/ci-dagger-sdk` reports `ready=true` after a miss. Readiness requires the three paths, a non-trivial `dagger.gen.go` (`package` line, ≥2KiB), at least one `.go` under `internal/dagger`, and ≥200KiB total on-disk (rejects the stuck ~59KiB Actions blob without demanding a 100KiB+ gen file). Incomplete hits regenerate and log a poison warning — bump the `vN` prefix to abandon a stuck key (Actions cannot overwrite). Warm check: Generate logs `dagger-develop-cache-hit`. Invalidate when any of these change: `dagger.json`, `.dagger-version`, `scripts/dagger-checksums.txt`, `runner/go.mod`, `runner/go.sum`, `runner/toolchain.json`, `runner/*.go`, `sdk/patched-go/**`. `vulnerabilities.yml` still always runs `scripts/test-sdk-security`.
