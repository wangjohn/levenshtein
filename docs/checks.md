...
- **The copy.** The user's working tree is never written. On the Dagger path the container's own copy of the imported source is the scratch tree. Natively, the check copies the target's declared inputs, less its `exclude` paths and private files (`.git`, `.env`), into a temporary directory that is removed afterwards; with git [discovery](configuration.md#input-discovery) it copies the files the fingerprint covers, which leaves out build output the repository ignores. Generators run with the copy as their working tree, `GOWORK` pointing at the copy's `go.work` when the target uses one, and `LEVENSHTEIN_SOURCE` at the copy. They see no repository history: the comparison happens in a git repository that lives outside the copy, so a generator that runs `git describe` fails the same way on both executors.
