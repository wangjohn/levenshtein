...
Turning a default rule off hides every finding it would report in the repository, including future ones. For one site, prefer `//lint:ignore <code> <reason>` on that line, which keeps the rule on everywhere else and records why. Staticcheck applies the directive for every rule, upstream analyzers included: `nilerr` reads `//lint:ignore nilerr` itself upstream, and here it reports the finding anyway so the directive suppresses it rather than being reported as matching nothing. A directive that matches no finding on its line is still reported as unused.
...
| `shell-lint` | ShellCheck's warnings and errors in the repository's shell scripts, one finding per diagnostic ([details](#shell-scripts)) | Repos with shell scripts |
| `secrets` | gitleaks' default rules over the repository's files, with every value redacted ([details](#secrets)) | Every repo, beside lint |
| `deps-vuln` | osv-scanner: known vulnerabilities in non-Go dependency lockfiles, one finding per package version ([details](#dependency-vulnerabilities)) | Repos with npm, Python, Rust, Ruby, or other lockfiles; dependency updates and daily |
...
Levenshtein's own `levenshtein.json` is a worked example of splitting executors: `branch` and `pre-merge` bind `go-lint`, `go-vet`, `go-mod`, `go-imports`, `workflow-lint`, `workflow-security`, `shell-lint`, and `secrets` to a [native environment](configuration.md#native-go-checks) for speed, and `main` keeps the same kinds in Dagger as the daily hermetic audit. Its `go-mod` check covers the root module, `runner/lint`, and `runner/tools`, but not `runner`, whose `go.mod` `dagger develop` rewrites on every regeneration. Its [import rules](#import-boundaries) keep `internal/gitchange` on the standard library alone, `internal/semantic` out of `internal/verify` and Dagger, and the CLI talking to `internal/verify` only; in `runner/lint`, nothing may import the Dagger SDK, the house rules depend on `golang.org/x/tools/go/...` alone, and `levenshtein-gocheck` on `golang.org/x/mod`.

A check with [`targets`](configuration.md#one-check-several-targets) plans one check per target: `lint` becomes `lint/api` and `lint/worker`, while `http/api` selects a single target. Each Go check runs for its selected target. Use a repository-root target (`dir: "."`) for `workflow-lint`, `workflow-security`, `shell-lint`, `secrets`, and `deps-vuln`, and choose what they read with its `inputs` and `exclude`; a workflow-less repo should omit the workflow kinds. A repository without `levenshtein.json` gets these checks over a single whole-tree target. HTTP and SQL checks do not replace application tests. ShellCheck and Pyflakes integration is explicitly disabled in `workflow-lint` so results do not depend on optional host tools; `shell-lint` checks standalone scripts with a pinned ShellCheck, but not workflow `run:` blocks.
...
The optional `apidiff` object accepts `base`, the branch whose merge base is compared with. It defaults, like [`go-mutation`'s](mutation.md#configure), to `GITHUB_BASE_REF`, else `main`, and resolves to `origin/<base>` unless the local branch is the same commit or ahead of it ([how](mutation.md#configure)).
...
`shell-lint` runs [ShellCheck](https://www.shellcheck.net) 0.11.0 over the repository's shell scripts: every `*.sh` and `*.bash` file, and every file without an extension whose first line is a `sh`, `bash`, `dash`, or `ksh` shebang, directly or through `env` (`#!/bin/sh`, `#!/usr/bin/env bash`, `#!/usr/bin/env -S bash -e`), among the files under the target's `inputs` and outside its `exclude`. It fails on ShellCheck's `warning` and `error` levels, with one finding per diagnostic carrying ShellCheck's code (such as `SC2164`), message, file, line, and column. Like `workflow-security`, it requires a repository-root target and is not in the default `branch`, `pre-merge`, or `main` gates: a repository without `levenshtein.json` can run it by name (`verify shell-lint`), and a configured one adds it to its own runs. Levenshtein runs it over `scripts/` and `verify`; nothing fired, at any level down to `style`.

- **Scope.** Scripts under a directory named `testdata`, `vendor`, or `node_modules` are never checked, at any depth, even when an input names one. The go command skips `testdata` the same way, so a repository can keep deliberately broken scripts as fixtures there, and vendored or installed scripts are not the repository's own to fix; use `exclude` for anything else. Both executors apply the same rules to the same files: the Dagger path lists its imported source in the container, and the native path walks the same inputs. Shell in workflow `run:` blocks is not a file, so `shell-lint` does not see it.
- **Severity.** ShellCheck's `info` and `style` levels, such as `SC2086` (an unquoted expansion) and `SC2006` (backticks), are not reported: they are conventions a repository adopts for itself, not defects a shared gate should impose on every consumer. `warning` covers mistakes that change what a script does, such as a `cd` whose failure is ignored (`SC2164`), an unused variable that is usually a typo (`SC2034`), or a bash construct under `#!/bin/sh` (`SC3010`); `error` covers scripts that do not parse or cannot work, such as `local` outside a function (`SC2168`). The threshold is fixed; run ShellCheck directly for a stricter audit.
- **Configuration.** The check honors one ShellCheck configuration at the root, `.shellcheckrc` or `shellcheckrc`, the names ShellCheck itself discovers, and passes it with `--rcfile`, so it applies to every script and a nested `.shellcheckrc` never does. More than one is an error. Without one the check passes `--norc`, so `~/.shellcheckrc` on a native host never changes the result; natively, `SHELLCHECK_OPTS` is removed from the environment as well. Use the file's `disable=` for a repository-wide exception, or a `# shellcheck disable=SC2034` directive above a line for one place. As with `workflow-security`, only a file under the target's `inputs` and outside its `exclude` is read, so declare it there: a change to it then invalidates a cached result.
- **Exit codes.** ShellCheck exits 1 when it reports a diagnostic, and the check requires its `json1` report to agree. Exit 2 (a script could not be read), 3 (bad command-line syntax), 4 (an unrecognized option), a report that is not JSON, and a report that contradicts the exit code are all errors, never a pass. A target without any scripts is an error too.
- **Caching.** The verdict depends only on the scripts, the configuration, and the pinned binary, so a `shell-lint` result is reused like `workflow-security`'s, and a fresh run (`rerun_checks`) executes it again.
- **The binary.** ShellCheck is a Haskell program, so it is not built from `runner/tools/go.mod`. `runner/toolchain.json` pins the upstream `.tar.gz` release asset for Linux and macOS on amd64 and arm64 by SHA-256. Upstream publishes no checksum file, so the digests were computed from the downloaded assets and cross-checked: the binary in each `.tar.gz` is byte-identical to the one in the matching `.tar.xz`, whose digest [shellcheck-py](https://github.com/shellcheck-py/shellcheck-py) records independently. The Dagger path fetches the asset for the engine's platform with that checksum, which the engine enforces before the file is used; the native path downloads it into its tool directory (under `--cache-dir` when one is set, so it is fetched once), checks the hash before extracting, and hashes the kept archive again on every later run. A platform without a pinned asset is an error, never an unverified download. Both paths need to reach `github.com`.

### Secrets

`secrets` runs [gitleaks](https://github.com/gitleaks/gitleaks) 8.30.1 with its default rules over the files under the target's `inputs` and outside its `exclude`, and fails on every leak it finds, with one finding per leak coded by the gitleaks rule that matched (such as `generic-api-key` or `aws-access-token`) at its file, line, and column. It requires a repository-root target and is not in the default `branch`, `pre-merge`, or `main` gates: run it by name (`verify secrets`) or add it to a run of your own. On Levenshtein it found three made-up keys, all written for its own tests: two test constants now carry `gitleaks:allow`, and its own check excludes the deliberately leaky fixture, `runner/testdata/secrets-leaky`.

- **Files, not history.** The check scans the working tree's files with gitleaks' directory mode (`gitleaks dir`), since the source Dagger imports has no `.git`. A secret that was committed stays in history after it is removed from the tree, so rotate it; a repository that needs a history audit runs `gitleaks git` in a job of its own. `.env` and `.env.*` files other than `.env.example` are never scanned: Levenshtein never imports them into Dagger, because they are meant to hold local secrets and stay out of version control, and the native path skips them to match. Symlinks are not followed. Test fixtures are scanned like everything else, since a fake credential in `testdata` still reads as a credential to anyone who finds it; allowlist one as below.
- **Redaction.** gitleaks runs with `--redact=100`, which replaces the value with `REDACTED` before it writes its report or a log line, and the check never reads the report's `Secret`, `Match`, or `Line` fields at all. A finding carries the rule's description, the location, and the gitleaks fingerprint, so no secret value reaches the report, the check's output, or the result cache. Tests on both executors assert that against a fixture key.
- **False positives.** From narrowest to broadest: a `gitleaks:allow` comment on the line; the finding's fingerprint (`path:rule-id:line`, which the finding's message quotes) as a line of a root `.gitleaksignore`; or an `[[allowlists]]` entry, by path, regex, or stop word, in a root `.gitleaks.toml` that keeps `[extend] useDefault = true` so the default rules still apply. A fingerprint names a line, so it has to follow the line when it moves; an allowlist does not. `exclude` drops a whole directory from the scan.
- **Configuration.** gitleaks reads `.gitleaks.toml` and `.gitleaksignore` from the root of the directory it scans, and the check names a root `.gitleaks.toml` with `--config` as well. It scans exactly the files the Dagger path imports (natively, it copies them into a temporary directory first), so only files the target declares and does not exclude count: declare both files in `inputs`, and a change to either invalidates a cached result. Without a `.gitleaks.toml`, gitleaks uses its default rules; natively, `GITLEAKS_CONFIG` and `GITLEAKS_CONFIG_TOML` are removed from the environment, so a host setting cannot replace them.
- **Exit codes.** gitleaks exits 1 both for leaks, by default, and for a fatal error, so the check passes `--exit-code=3` and treats only 3, with a JSON report that lists leaks, as findings, and 0 with an empty report as a pass. Any other exit, a missing or unreadable report, and a report that disagrees with the exit code are errors, never a pass. A target without any files is an error too.
- **Caching.** The verdict depends only on the files, the configuration, and the pinned build, so a `secrets` result is reused like `shell-lint`'s.
- **The binary.** gitleaks is a Go program, pinned in `runner/tools/go.mod` (as `github.com/zricethezav/gitleaks/v8`, the module path upstream still publishes) and built on either executor the way `actionlint` and `govulncheck` are; adding it raised no version those tools are built with. Building it natively needs the Go module proxy until the build cache has it.

### Dependency vulnerabilities

`deps-vuln` runs [osv-scanner](https://google.github.io/osv-scanner/) 2.6.0 over the dependency lockfiles under the target's `inputs` and outside its `exclude`: npm, pnpm, yarn, and bun lockfiles; Python requirements, Pipfile, Poetry, PDM, pylock, and uv lockfiles; `Cargo.lock`, `Gemfile.lock`, `composer.lock`; the other formats osv-scanner's `lockfile` preset reads; and SPDX and CycloneDX SBOMs. It fails once per vulnerable package version: the finding's code is `deps-vuln`, its location is the lockfile, and its message names the package, version, and ecosystem and, for each vulnerability osv-scanner groups, the advisory IDs, their aliases, the highest severity score, and a summary. It requires a repository-root target and is not in the default `branch`, `pre-merge`, or `main` gates; like `go-vuln`, it belongs in a run for dependency changes and a daily one.

- **Scope.** As for `shell-lint`, lockfiles under a directory named `testdata`, `vendor`, or `node_modules` are never scanned, at any depth: a fixture that pins a vulnerable version on purpose is not a dependency, and an installed or vendored tree repeats what the repository's own lockfile already pins. The native path leaves them out of the directory it scans; the Dagger path, which scans `/src` in place, passes osv-scanner an `--experimental-exclude` pattern instead. The native path passes none, because osv-scanner matches the pattern against absolute paths, and a temporary directory under a `vendor` directory would exclude every lockfile. Use `exclude` for anything else.
- **Go modules are `go-vuln`'s.** osv-scanner would also read `go.mod`, but it matches versions only, while govulncheck reports what the code can reach; reporting Go modules here would repeat `go-vuln`'s findings and contradict its judgment that an unreachable vulnerability needs no action. The check disables osv-scanner's Go module extractor with `--experimental-disable-plugins=go/gomod`, and turns call analysis off for every language with `--no-call-analysis=all`, since it would otherwise run govulncheck, or for Rust the repository's own build scripts.
- **What else is off.** `--experimental-disable-plugins=directory` disables the directory preset, which identifies vendored C and C++ code by hashing it through the OSV API and reads git repositories. `--no-resolve` keeps a manifest such as `pom.xml` to the versions it names, rather than resolving transitive dependencies through deps.dev. `--no-ignore` scans lockfiles that `.gitignore` names, since the target's `inputs` and `exclude` already decide what is in scope.
- **Configuration.** A root `osv-scanner.toml` is passed with `--config` and applies to every lockfile; without one, osv-scanner reads the `osv-scanner.toml` beside each lockfile, if there is one. Either way, only files under the target's `inputs` and outside its `exclude` are visible, since the native path copies them into a temporary directory before scanning, as the Dagger path imports them. To accept a vulnerability, add an `[[IgnoredVulns]]` entry with its `id`, a `reason`, and optionally `ignoreUntil`; a root file that does not parse is an error. On a native host, `OSV_SCANNER_*` settings are removed from the environment.
- **Nothing to scan.** A target without a supported lockfile is an error (osv-scanner's exit 128), as a Go check without packages is, rather than an empty pass. A repository whose only dependencies are Go modules leaves `deps-vuln` out and runs `go-vuln`.
- **Exit codes and the network.** osv-scanner exits 1 when it finds vulnerabilities, and the check requires its JSON report to list them; 0 with no vulnerable package is a pass. An advisory osv-scanner lists but does not count toward that exit, such as a Debian advisory the distribution rates unimportant in an SBOM, is not reported either. Exit 128 is the error above, and 127 (a general error, including an advisory query that could not reach `api.osv.dev`) and every other exit are errors, never a pass. The native check reaches `api.osv.dev` with the check's own environment, so a host behind a proxy passes its settings with `pass_env`.
- **Caching.** Advisory data changes without any source change, so, like `go-vuln`, a `deps-vuln` result is never reused on either executor, each Dagger call gets a fresh nonce, and a direct Dagger `sharedCheck` call for `deps-vuln` without one is refused.
- **The binary.** osv-scanner is a Go program, but building it from `runner/tools/go.mod` would add about 190 modules and raise versions `actionlint` and `govulncheck` are built with, so `runner/toolchain.json` pins the upstream release binaries for Linux and macOS on amd64 and arm64 instead, by the SHA-256 digests upstream publishes in the release's `osv-scanner_SHA256SUMS`. They are fetched and verified the way ShellCheck is.
- **Levenshtein itself.** Levenshtein has no dependencies outside Go, so its own `levenshtein.json` leaves `deps-vuln` out. The only lockfiles it holds are the language fixtures under `tests/fixtures`, a uv project and a Cargo workspace that exist to exercise language contracts rather than to ship, which scan clean today, and the deliberately vulnerable `runner/testdata/deps-vulnerable`. Its files carry a `.fixture` suffix, so GitHub's dependency graph, Dependabot, and `dependency-review` never mistake them for Levenshtein's own dependencies; the tests copy them under their real names.
