...
- **Scope.** As for `shell-lint`, lockfiles under a directory named `testdata`, `vendor`, or `node_modules` are never scanned, at any depth: a fixture that pins a vulnerable version on purpose is not a dependency, and an installed or vendored tree repeats what the repository's own lockfile already pins. The native path leaves them out of the directory it scans, and both executors pass osv-scanner the same `--experimental-exclude` pattern. Use `exclude` for anything else.
