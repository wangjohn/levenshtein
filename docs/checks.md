...
- **Exit codes and the network.** osv-scanner exits 1 when it finds vulnerabilities, and the check requires its JSON report to list them; 0 with no vulnerable package is a pass. Exit 128 is the error above, and 127 (a general error, including an advisory query that could not reach `api.osv.dev`) and every other exit are errors, never a pass. The native check reaches `api.osv.dev` with the check's own environment, so a host behind a proxy passes its settings with `pass_env`.
