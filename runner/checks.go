...
// package at all. Every other failed package is a finding carrying its own
// output, which covers a failed or panicking test, a data race the race
// detector reported, and a test that hit the timeout. A run that passes without
// running a single test is refused rather than reported as a pass.
// internal/verify keeps a copy for the native executor; change both together.
